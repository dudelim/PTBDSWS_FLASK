...
def send_simple_message(to, subject, newUser):
    ...
    email_body = f"Novo usuário cadastrado: {newUser}\nNome: Maria Eduarda Melim\nProntuário: PT3025594"
...
def index():
    ...
    if form.validate_on_submit():
        ...
        if user is None:
            ...
            print('api: ' + str(app.config['API_KEY']), flush=True)
            print('from: ' + str(app.config['API_FROM']), flush=True)
            print('to: ' + str([app.config['FLASKY_ADMIN'], "maria.melim@aluno.ifsp.edu.br","flaskaulasweb@zohomail.com"]), flush=True)
            ...
            if app.config['FLASKY_ADMIN']:
                ...
                send_simple_message([app.config['FLASKY_ADMIN'], "maria.melim@aluno.ifsp.edu.br", "flaskaulasweb@zohomail.com"], 'Novo usuário', form.name.data)
